"""Command line entrypoint for wotbot process roles."""
...
def _serve(args: argparse.Namespace) -> None:
    ...
    if args.reload:
        command.extend(["--reload", "--reload-dir", "src/wotbot"])
    _exec(command)
...
def _job_worker(_args: argparse.Namespace) -> None:
    _exec(
        [
            ...
            "--ack-type",
            "when_saved",
            "wotbot.workers.job_worker:broker",
        ]
    )
...
def _job_scheduler(_args: argparse.Namespace) -> None:
    _exec(
        [
            ...
            "--loop-interval",
            "1",
            "wotbot.workers.job_scheduler:scheduler",
        ]
    )


def _thing_indexer(_args: argparse.Namespace) -> None:
    from wotbot.workers.thing_indexer import run
...
def _livekit_agent(_args: argparse.Namespace) -> None:
    from wotbot.workers.livekit import run

    run(_args.livekit_args)


def main(argv: Sequence[str] | None = None) -> None:
    parser = argparse.ArgumentParser(prog="wotbot")
