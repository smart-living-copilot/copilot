...
async function ensureGroup(): Promise<void> {
  ...
  await client
    .xgroup('CREATE', config.thingEventsStream, config.thingEventsGroup, '$', 'MKSTREAM')
    .catch((error: any) => {
...
export async function handleEvent(fields: Record<string, string>): Promise<void> {
  ...
  await reconcileThingId(String(event.id || ''), String(event.action || 'update'));
}
      ...
      try {
        ...
        for (const stream of (response || []) as [string, [string, string[]][]][]) {
          for (const [id, fields] of stream[1] as [string, string[]][]) {
            ...
            await client.xack(config.thingEventsStream, config.thingEventsGroup, id);
          }
        }
      } catch (error) {
        if (!stopped) {
