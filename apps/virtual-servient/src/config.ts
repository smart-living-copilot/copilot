...
function registryTokenEnv(): string {
  ...
  throw new Error(
    "Missing required environment variable: VIRTUAL_SERVIENT_REGISTRY_TOKEN",
  );
}
...
export const config = {
  ...
  thingEventsGroup:
    process.env.VIRTUAL_SERVIENT_EVENTS_GROUP || "virtual_servient",
  thingEventsConsumer:
    process.env.VIRTUAL_SERVIENT_EVENTS_CONSUMER ||
    `virtual-servient-${process.pid}`,
  requestTimeoutMs: parseIntEnv("HTTP_REQUEST_TIMEOUT_MS", 10000),
  reconcileIntervalMs: parseIntEnv(
    "VIRTUAL_SERVIENT_RECONCILE_INTERVAL_MS",
    30000,
  ),
} as const;
